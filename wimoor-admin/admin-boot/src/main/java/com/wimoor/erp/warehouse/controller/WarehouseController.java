...
public class WarehouseController {
	...
	public Result<List<Warehouse>> findWarehouseFtypeList(String ftype,HttpServletRequest request){
		...
		for(Warehouse item:list) {
			if(StrUtil.isBlank(item.getParentname())){
				Warehouse parent = warehouseService.getById(item.getParentid());
				item.setParentname(parent!=null?parent.getName():null);
			}
			if(ftype.contains("usable")&&item.getName().equals("正品仓")&&StrUtil.isNotBlank(item.getParentname())){
				item.setName(item.getName().replace("正品仓", item.getParentname()));
			}
			if(ftype.contains("test")&&item.getName().equals("测试仓")&&StrUtil.isNotBlank(item.getParentname())){
				item.setName(item.getName().replace("测试仓", item.getParentname()));
			}
			if(ftype.contains("unusable")&&item.getName().equals("废品仓")&&StrUtil.isNotBlank(item.getParentname())){
				item.setName(item.getName().replace("废品仓", item.getParentname()));
			}
		}
		return Result.success(list);
	}
	...
	public Result<Warehouse> updateDataAction(@ApiParam("查询当前仓库") @PathVariable("id") String id, @RequestBody Warehouse model,HttpServletRequest request) {
		...
		if (!model.getFtype().contains("over") && (model.getStockingCycle() == null || model.getStockingCycle() < 0)) {
