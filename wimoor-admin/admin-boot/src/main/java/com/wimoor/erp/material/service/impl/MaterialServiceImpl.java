...
public class MaterialServiceImpl extends  ServiceImpl<MaterialMapper,Material> implements IMaterialService {
	...
	public List<String> getmskuList(String shopid,List<String> list) {
		...
		param.put("list", list);
		return materialTagsMapper.getmskuList(param);
	}
	...
	public List<String> getmskuListWithout(String shopid,List<String> list) {
		...
		param.put("shopid", shopid);
		param.put("list", list);
		return materialTagsMapper.getmskuListWithout(param);
	}
	...
	List<MaterialCustoms> saveMaterialCustoms(MaterialInfoVO vo,Material material){
		...
		if(customsvo!=null && customsvo.size()>0) {
	...
	public String saveAllInfo(MaterialInfoVO vo,MultipartFile file, UserInfo user) throws ERPBizException {
 		...
 		if(vo!=null&&vo.getMaterial()!=null&&vo.getMaterial().getSku()!=null) {
...
public List<MaterialCustoms> selectCustomsByMaterialId(String id) {
	LambdaQueryWrapper<MaterialCustoms> query=new LambdaQueryWrapper<MaterialCustoms>();
	query.eq(MaterialCustoms::getMaterialid, id);
	return materialCustomsMapper.selectList(query) ;
}
...
public Map<String, Object> getRealityPrice(String materialid){
  ...
  public void savetofile(InputStream inputStream) {
	...
	public List<Map<String, Object>> findInventoryByMsku(PlanDTO dto,String key) {
		...
		if(dto.getMskulist()!=null&&dto.getMskulist().size()==0){
			dto.setMskulist(null);
		}
		...
		if(records!=null && records.size()>0) {
			...
			if("MaterialCustoms".equals(dto.getDowntype())) {
				...
				for(int i=0;i<records.size();i++) {
					...
					for(int j=0;j<customs.size();j++) {
						...
						cel0.setCellValue(sku);
						if(customs!=null) {
							Cell cell1 = row.createCell(1);
							cell1.setCellValue(custom.getCountry());
							Cell cell2 = row.createCell(2);
							cell2.setCellValue(custom.getCname());
							Cell cell3 = row.createCell(3);
							cell3.setCellValue(custom.getEname());
							Cell cell4 = row.createCell(4);
							cell4.setCellValue(custom.getMaterial());
							Cell cell5 = row.createCell(5);
							cell5.setCellValue(custom.getMaterialcn());
							Cell cell6 = row.createCell(6);
							cell6.setCellValue(custom.getApplication());
							Cell cell7 = row.createCell(7);
							cell7.setCellValue(custom.getPrice()!=null?custom.getPrice().toString():"");
							Cell cell8 = row.createCell(8);
							cell8.setCellValue(custom.getCode());
			...
			if("MaterialBaseInfo".equals(dto.getDowntype())
				...
				for(int i=0;i<records.size();i++) {
					...
					cell.setCellValue(sku);
						Cell cell2 = row.createCell(1);
						cell2.setCellValue(item.get("name")!=null?item.get("name").toString():"");
						...
						if(!"MaterialBaseInfoConsumable".equals(dto.getDowntype())) {
			...
			for (MaterialVO item:volist) {
				if(item.getId()!=null) {
					...
					if(material!=null) {
						...
						if("dim".equals(ftype)) {
							DimensionsInfoVo dim = item.getPkgdimension();
							if(dim!=null){
								if(material.getPkgdimensions()!=null){
									DimensionsInfo dimensionsInfo = dimensionsInfoService.getById(material.getPkgdimensions());
									dimensionsInfo.setHeight(dim.getHeight());
									dimensionsInfo.setLength(dim.getLength());
									dimensionsInfo.setWidth(dim.getWidth());
									dimensionsInfo.setWeight(dim.getWeight());
									dimensionsInfoService.updateById(dimensionsInfo);
								}else{
									...
									dimensionsInfo.setWidth(dim.getWidth());
									dimensionsInfo.setWeight(dim.getWeight());
									dimensionsInfoService.save(dimensionsInfo);
									material.setPkgdimensions(dimensionsInfo.getId());
								}
							}
						}
						...
						if("tags".equals(ftype)) {
							this.saveTagsByMid(material.getId(), item.getTaglist(), user.getId());
							continue;
						}
						material.setOperator(user.getId());
						this.baseMapper.updateById(material);
	...
	@Override
	public List<Map<String, Object>> getDimensions(String shopid, List<String> skulist) {
		// TODO Auto-generated method stub
		Map<String,Object> param=new HashMap<String,Object>();
		param.put("shopid", shopid);
		param.put("skulist", skulist);
		if(skulist!=null&&skulist.size()>0) {
			return this.baseMapper.getDimensions(param);
		}else {
			return null;
		}
	}
	...
	@Override
	public void saveCustoms(UserInfo user,List<MaterialCustoms> list) {
		 for(MaterialCustoms item:list) {
			 LambdaQueryWrapper<MaterialCustoms> query=new LambdaQueryWrapper<MaterialCustoms>();
			 ...
			 if(old!=null) {
				 ...
				 item.setCreator(old.getCreator());
				 materialCustomsMapper.update(item, query);
			 }else {
				 item.setOperator(user.getId());
				 item.setOpttime(new Date());
