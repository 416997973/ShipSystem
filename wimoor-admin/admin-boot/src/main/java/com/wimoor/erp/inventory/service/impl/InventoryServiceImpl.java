...
public class InventoryServiceImpl  extends ServiceImpl<InventoryMapper,Inventory> implements IInventoryService {
	...
	private void setInitRecord(InventoryParameter para,InventoryRecord newRecord, Status status, Operate operate) {
		newRecord.setId(warehouseService.getUUID());
	...
	void insertHis(Inventory addinv) throws BizException {
		...
		if (invhis != null) {
			...
			his.setId(warehouseService.getUUID());
	...
	public int SubStockByStatus(InventoryParameter para, Status status, Operate operate) throws BizException {
		...
		try {
				...
				//库存初始的量
				InventoryRecord recform=new InventoryRecord();
				recform.setId(warehouseService.getUUID());
				handleStatusRecord(para,recform,"start");
				Warehouse warehouseobj = isOkForOperate(para.getWarehouse(), para.getStatus());
	...
	private Inventory setInitInventory(InventoryParameter para) {
		...
		newInv.setId(warehouseService.getUUID());
	...
	public MaterialInventoryVo findMaterialsInventory(String shopid,List<String> materialids,String warehouseid) {
		Map<String,Object> param=new HashMap<String,Object>();
		param.put("shopid",shopid);
		param.put("materialids",materialids);
		param.put("warehouseid",warehouseid );
		return this.baseMapper.findMaterialsInventory(param);
	}
	...
	@Override
	public InventoryVo getInventory( String materialid, String warehouseid,String shopid) {
		// TODO Auto-generated method stub
		Map<String, Object> map = this.baseMapper.getInventory(materialid, warehouseid, shopid);
		if(map==null)return null;
		else {
			InventoryVo vo=new InventoryVo();
			if(map.get("fulfillable")!=null) {
				vo.setFulfillable(Integer.parseInt(map.get("fulfillable").toString()));
			}
			if(map.get("outbound")!=null) {
				vo.setOutbound(Integer.parseInt(map.get("outbound").toString()));
			}
			if(map.get("inbound")!=null) {
				vo.setInbound(Integer.parseInt(map.get("inbound").toString()));
			}
			return vo;
		}
	}
	...
	public void repairInventory(UserInfo user,String warehouseid,String materialid,String status,Integer qty){
		    Inventory inv = this.baseMapper.selectNowInv(warehouseid, materialid, user.getCompanyid(), status);
			...
			String statustype=statusarray[0];
		    String formtype=statusarray[1];
			InventoryParameter para = new InventoryParameter();
			para.setFormid(user.getId());
			para.setFormtype(formtype);
			para.setMaterial(materialid);
			para.setOperator(user.getId());
			para.setOpttime(new Date());
			para.setNumber("NA");
			para.setShopid(user.getCompanyid());
			para.setStatus(EnumByInventory.Ready);
			para.setWarehouse(warehouseid);
			if(inv!=null&&inv.getQuantity()>qty){
				para.setAmount(inv.getQuantity()-qty);
				SubStockByStatus(para,Status.valueOf(statustype), Operate.changesub);
			}else if(inv!=null){
				para.setAmount(qty-inv.getQuantity());
				AddStockByStatus(para, Status.valueOf(statustype), Operate.changeadd);
			}else{
				para.setAmount(qty);
				AddStockByStatus(para, Status.valueOf(statustype), Operate.changeadd);
			}
