...
public class PurchasePlanServiceImpl extends  ServiceImpl<PurchasePlanMapper,PurchasePlan> implements IPurchasePlanService {
	...
	public Map<String,Object> getLastForms(List<String> ids) {
	    ...
	    Map<String,Date> resultDate=new HashMap<String,Date>();
		...
		for(Map<String, Object> data:datas) {
			if (data != null) {
				...
				Date time =null;
				if (creatdate != null) {
					time = GeneralUtil.getDate(creatdate);
					lastform =GeneralUtil.formatDate(time)  ;
				}
				lastform = lastform + "  " + data.get("amount");
				Object auditstatus = data.get("auditstatus");
				if (auditstatus != null) {
					lastform = lastform + " <br> " + PurchaseFormEntry.getAuditstatusName(Integer.parseInt(auditstatus.toString()));
				}
				result.put(materialid, lastform);
				resultDate.put(materialid, time);
			}
		}
		...
		for(AssemblyForm form:forms) {
			if (form != null) {
				...
				Date time = resultDate.get(form.getMainmid());
				if(time==null||time.before(form.getCreatedate())) {
	...
	public Map<String, Object> getLast3Form(Map<String, Object> map) {
		...
		if ("1".equals(issfg.toString())) {
			List<Map<String,Object>> formlist = assemblyFormService.getLastThreeFormByMaterial(id);
