...
public class WarehouseShelfInventoryController {
	...
	//导出当前库位中暂存库存的情况
	@PostMapping("/downloadShelfList")
	public void downloadShelfListAction(@RequestBody ShelfInvListDto condition,HttpServletResponse response){
		UserInfo user = UserInfoContext.get();
		...
		try {
			...
			response.addHeader("Content-Disposition", "attachment;fileName=stockShelfList" + System.currentTimeMillis() + ".xlsx");// 设置文件名
			ServletOutputStream fOut = response.getOutputStream();
			// 创建新的Excel工作薄
			SXSSFWorkbook workbook = new SXSSFWorkbook();
			iWarehouseShelfInventoryService.downloadShelfList(workbook, param);
			workbook.write(fOut);
			fOut.flush();
			fOut.close();
		} catch (IOException e) {
	...
	//导出当前库位中已上架的情况
	@PostMapping("/downloadShelfInventoryList")
	public void getListDetailExportAction(@RequestBody ShelfInvListDto condition,HttpServletResponse response){
		UserInfo user = UserInfoContext.get();
		Map<String, Object> param=new HashMap<String, Object>();
		String shopid = user.getCompanyid();
		String search=null;
		...
		try {
			response.setContentType("application/force-download");// 设置强制下载不打开
			response.addHeader("Content-Disposition", "attachment;fileName=stockShelfDetail" + System.currentTimeMillis() + ".xlsx");// 设置文件名
			ServletOutputStream fOut = response.getOutputStream();
			// 创建新的Excel工作薄
			SXSSFWorkbook workbook = new SXSSFWorkbook();
			iWarehouseShelfInventoryService.downloadShelfInventoryList(workbook, param);
			workbook.write(fOut);
			fOut.flush();
			fOut.close();
		} catch (IOException e) {
