...
public class MaterialController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(name = "page", value = "页码", paramType = "query", dataType = "Long"), @ApiImplicitParam(name = "limit", value = "每页数量", paramType = "query", dataType = "Long"), @ApiImplicitParam(name = "search", value = "模糊查询", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "ftype", value = "查询类型当为shipment时只查询库存大于0的", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "isDelete", value = "是否逻辑删除", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "searchlist", value = "SKU批量查询", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "issfg", value = "组装类型", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "warehouseid", value = "仓库ID", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "categoryid", value = "产品分类ID", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "supplier", value = "供应商ID", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "owner", value = "负责人ID", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "name", value = "产品名称", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "remark", value = "产品备注", paramType = "query", dataType = "String"), @ApiImplicitParam(name = "color", value = "颜色标签对应的颜色", paramType = "query", dataType = "String")})
    ...
    public Result<IPage<Map<String, Object>>> list(@RequestBody MaterialDTO dto) {
        ...
        map.put("withoutTags", dto.getWithoutTags() != null && dto.getWithoutTags());
        map.put("withPriceHis", dto.getWithPriceHis() != null && dto.getWithPriceHis());
    ...
    public Result<IPage<Map<String, Object>>> consumableList(@RequestBody MaterialDTO dto) {
        ...
        map.put("ftype", ftype);
        map.put("isDelete", isDelete);
        map.put("isplan", dto.getIsplan() != null && dto.getIsplan());
        if (dto.getIswarning() != null && dto.getIswarning()) {
    ...
    public Result<MaterialInfoVO> getMaterialInfoAction(@ApiParam("产品ID") @RequestParam String id) {
        ...
        if (taglist != null && taglist.size() > 0) {
            for (int i = 0; i < taglist.size(); i++) {
                tagliststr += (taglist.get(i) + ",");
                tagsIdsList.add(taglist.get(i));
            }
        }
        try {
...
//		} 
    ...
    public Result<String> getSerialNumber(String ftype, String isfind, HttpServletRequest request) {
                ...
                if ("2".equals(issfg)) {
                    ...
                    if (asslist != null && asslist.size() > 0) {
                        for (Assembly assemblymain : asslist) {
                            ...
                            if (mainMaterial == null || mainMaterial.isDelete()) {
                                continue;
                            }
                ...
                if ("1".equals(issfg)) {
                    ...
                    if (subasslist != null && subasslist.size() > 0) {
                        for (Assembly subitem : subasslist) {
                            ...
                            if (!issub && subMaterial.getIssfg().equals("2")) {
        ...
        if (material.getIssfg().equals("1")) {
            ...
            if (subasslist != null && subasslist.size() > 0) {
                for (Assembly subitem : subasslist) {
                    ...
                    if (issub && subMaterial.getIssfg().equals("0")) {
    ...
    public Result<Material> getMSkuDeliveryAndInv(@RequestParam String shopid, @RequestParam String groupid, @RequestParam String msku, @RequestParam String country, @RequestParam String needDeliveryCycle) {
