   ...
   public int restartRec(UserInfo user,PurchaseFormEntry entry) {
	...
	public PurchaseFormEntry clearReceiveItem(UserInfo user,String recid) {
		...
		if(purchaseFormEntry.getInwhstatus()==1) {
	     ...
	     Integer totalin=0;
		for(PurchaseFormReceive myitem:reclist) {
			if("in".equals(myitem.getFtype())) {
				totalin=totalin+myitem.getAmount();
			}else if("out".equals(myitem.getFtype())) {
				totalin=totalin-myitem.getAmount();
			}
		...
		if("in".equals(item.getFtype())) {
			...
			int nowtotalin=totalin+item.getAmount();
			if (totalin<purchaseFormEntry.getAmount()) {
				  if(nowtotalin<=purchaseFormEntry.getAmount()) {
					    invpara.setAmount(item.getAmount());
					    invpara.setWarehouse(form.getWarehouseid());
					    inventoryService.SubStockByStatus(invpara, Status.inbound, Operate.in);
				  }else {
					  int needinbound=purchaseFormEntry.getAmount()-totalin;
					  invpara.setAmount(needinbound);
					  invpara.setWarehouse(form.getWarehouseid());
					  inventoryService.SubStockByStatus(invpara, Status.inbound, Operate.in);
				  }
	...
	public PurchaseFormEntry clearReceive(String entryid, UserInfo user) {
		...
		if (amount > 0) {
			...
			for(PurchaseFormReceive item:reclist) {
				if("in".equals(item.getFtype())) {
					...
					item.setRemark("已撤销【通过撤销入库操作】");
					this.baseMapper.updateById(item);
					totalin=totalin+item.getAmount();
				}else if("out".equals(item.getFtype())) {
					invpara.setWarehouse(item.getWarehouseid());
					invpara.setAmount(item.getAmount());
					inventoryService.AddStockByStatus(invpara, Status.fulfillable, Operate.in);
		...
		if (totalin > 0) {
			invpara.setAmount(totalin);
		} else {
			throw new BizException("本次操作数量超出单据入库范围，请单笔操作");
		}
	...
	@Override
	public Map<String, Object> receiveInfo(UserInfo userinfo,String id) {
		Map<String,Object> result=this.baseMapper.receiveInfo(id);
		if(result==null){
			throw new BizException("无法找到对应入库记录");
		}
		List<WarehouseShelfTreeVo> shelfvo = iWarehouseShelfService.getAllTree(userinfo, result.get("warehouseid").toString());
		result.put("shelftree", shelfvo);
		return result;
	}
