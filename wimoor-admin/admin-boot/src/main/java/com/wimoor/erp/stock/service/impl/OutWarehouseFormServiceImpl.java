	...
	public String uploadOutStockByExcel(Sheet sheet, UserInfo user) throws BizException {
		...
		if (remarkcell != null) {
			try {
				...
				Date date = DateUtil.getJavaDate(remark_);
			...
			if(StrUtil.isBlank(remark)){
				...
				remark=remark+";Excel导入数据。";
			}

		}
		Row row6 = sheet.getRow(6);
		Cell formtypecell = row6.getCell(1);
		if (formtypecell != null) {
				formtype = formtypecell.getStringCellValue();
				Map<String, String> mapnamevalue = adminClientOneFeignManager.listType("out_form_type");
				formtype=mapnamevalue.get(formtype);
		}
		if(formtype==null) {
			formtype="0";
		}
		if (GeneralUtil.isNotEmpty(whname)) {
			...
			if(formtype.equals("0")) {//正品出库
				queryWrapper.eq("ftype", "self_usable");
			}else if(formtype.equals("1")) {//废品出库
				queryWrapper.eq("ftype", "self_unusable");
			}else if(formtype.equals("2")) {//验收出库
				queryWrapper.eq("ftype", "self_test");
			}else if(formtype.equals("3")) {//海外出库
				queryWrapper.eq("ftype", "oversea_usable");
			}
