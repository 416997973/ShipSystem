...
public class PurchaseFormPaymentServiceImpl extends  ServiceImpl<PurchaseFormPaymentMapper,PurchaseFormPayment> implements IPurchaseFormPaymentService {
	...
	private void saveRecord(FinAccount account, PurchaseFormPayment payment, PurchaseFormPayment oldpayment, UserInfo user) {
		...
		if(account.getIsautopay()!=null&&account.getIsautopay()==true) {
			if(account.getProjectMap()==null) {
				account.setProjectMap(new HashMap<String,BigDecimal>());
			}
			Map<String, BigDecimal> project = account.getProjectMap();
			if(project.get(projectid)==null) {
				project.put(projectid, new BigDecimal("0"));
			}
			BigDecimal projectvalue = project.get(projectid);
			amount=amount.add(projectvalue);
			project.put(projectid,amount);
			return ;
		}
