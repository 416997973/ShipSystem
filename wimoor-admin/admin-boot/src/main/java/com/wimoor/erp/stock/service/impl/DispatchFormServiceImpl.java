...
import com.wimoor.erp.stock.mapper.DispatchFormRecordMapper;
...
public class DispatchFormServiceImpl  extends  ServiceImpl<DispatchFormMapper,DispatchForm> implements IDispatchFormService {
    ...
    final DispatchFormRecordMapper dispatchFormRecordMapper;
	...
	public IPage<Map<String, Object>> findByCondition(Page<?> page,Map<String, Object> map) {
		...
		IPage<Map<String, Object>> list = dispatchFormMapper.findByCondition(page, map);
		for(Map<String, Object> item:list.getRecords()){
			List<DispatchFormRecord> records = dispatchFormRecordMapper.selectList(new LambdaQueryWrapper<DispatchFormRecord>().eq(DispatchFormRecord::getFormid, item.get("id")));
			if(records!=null&&records.size()>0){
				for(DispatchFormRecord record:records){
					if(record.getOperator()!=null){
						record.setOperator(namemap.get(record.getOperator()));
					}
				}
			}
	...
	public String uploadDispatchStockByExcel(Sheet sheet, UserInfo user)   {
		...
		if(wh_out.getFtype().equals("self_usable")&&wh_in.getFtype().equals("self_unusable")) {
			dispatchForm.setFtype(1);
		}
		if(wh_out.getFtype().equals("self_unusable")&&wh_in.getFtype().equals("self_usable")) {
			dispatchForm.setFtype(2);
		}
		if(wh_out.getFtype().equals("self_test")&&wh_in.getFtype().equals("self_usable")) {
			dispatchForm.setFtype(4);
		}
		if(wh_out.getFtype().equals("self_usable")&&wh_in.getFtype().equals("self_test")) {
			dispatchForm.setFtype(3);
		}
		if(wh_out.getFtype().equals("self_usable")&&wh_in.getFtype().equals("self_usable")) {
			dispatchForm.setFtype(0);
		}
		if(wh_out.getFtype().equals("oversea_usable")&&wh_in.getFtype().equals("self_usable")) {
			dispatchForm.setFtype(6);
		}
