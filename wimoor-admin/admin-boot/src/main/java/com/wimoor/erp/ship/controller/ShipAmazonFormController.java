...
public class ShipAmazonFormController {
	...
	public Result<ShipInboundShipmenSummarytVo> quotainfoAction(@PathVariable("shipmentid") String shipmentid) {
		    ...
		    for(ShipInboundItemVo item:itemsum.getItemList()) {
		    	MaterialVO material = this.materialService.findMaterialById(item.getMaterialid());
				if(material!=null&&material.getImage()!=null) {
					  item.setImage(material.getImage());
				} 
		    }
			if(StrUtil.isBlank(itemsum.getCheckinv())) {
				itemsum.setCheckinv("0");
			}
	...
	public Result<ShipInboundShipmenSummarytVo> quotainfosAction(@RequestBody List<String> shipmentid) {
		    Result<ShipInboundShipmenSummarytVo> result = amazonClientOneFeign.infosAction(shipmentid);
		    ShipInboundShipmenSummarytVo itemsum=result.getData();
		    for(ShipInboundItemVo item:itemsum.getItemList()) {
		    	MaterialVO material = this.materialService.findMaterialById(item.getMaterialid());
				if(material!=null&&material.getImage()!=null) {
	...
	public void downPDFShipFormAction(@PathVariable("ftype") String ftype,
			@ApiParam("货件ID")@RequestBody ShipPrintLabelDTO dto,
			HttpServletResponse response) {
		UserInfo user=UserInfoContext.get();
		response.setContentType("application/force-download");// 设置强制下载不打开
		response.addHeader("Content-Disposition", "attachment;fileName=shipForm" + System.currentTimeMillis() + ".pdf");// 设置文件名
		...
		// step 2
		try {
		    Result<ShipInboundShipmenSummarytVo> result = amazonClientOneFeign.infosAction(dto.getShipments());
		    ShipInboundShipmenSummarytVo itemsum=result.getData();
		    for(ShipInboundItemVo item:itemsum.getItemList()) {
		    	MaterialVO material = this.materialService.findMaterialById(item.getMaterialid());
		    	if(material==null) {
		    		 ...
		    		 if(m!=null) {
		    			 ...
		    			 Picture p = this.iPictureService.getById(m.getImage());
		    			 if(p!=null) {
		    				 item.setImage(p.getLocation());
		    			 }
		    		 }
		    	}
				if(material!=null&&material.getImage()!=null) {
		...
		try {
		   	 ...
		   	 for (int i = 0; i < shipmentobj.getItemList().size(); i++) {
			     ...
			     item.setUnitcost(price.add(shipfee));
		   	    ...
		   	    if(ShipInboundShipmentDTO.NeedInv(shipmentobj)) {
    ...
    public  void getQRCode(@ApiParam("货件ID") @PathVariable("shipmentid") String shipmentid,
    	...
    	String content = "wxsc/"+shipmentid;//内容信息
